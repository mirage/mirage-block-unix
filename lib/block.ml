...
let src =
  let src =
    Logs.Src.create "mirage-block-unix" ~doc:"Mirage BLOCK interface for Unix"
  in
...
module Raw = struct
  ...
  external writev_job :
    Unix.file_descr -> (buffer * int * int) list -> int Lwt_unix.job
    = "mirage_block_unix_writev_job"

  external readv_job :
    Unix.file_descr -> (buffer * int * int) list -> int Lwt_unix.job
    = "mirage_block_unix_readv_job"

  external iov_len : unit -> int = "mirage_block_unix_get_iov_len"

  external chsize_job : Unix.file_descr -> int64 -> unit Lwt_unix.job
    = "mirage_block_unix_chsize_job"

  external flock : Unix.file_descr -> bool (* ex *) -> bool (* nb *) -> unit
    = "stub_flock"
end

let ftruncate fd size =
  if is_win32 then
    Lwt_unix.run_job (Raw.chsize_job (Lwt_unix.unix_file_descr fd) size)
  else Lwt_unix.LargeFile.ftruncate fd size

open Lwt

module Config = struct
  type sync_behaviour = [ `ToOS | `ToDrive ]
  ...
  let of_string x =
    ...
    | Some "file" ->
        ...
        let buffered =
          try List.assoc "buffered" query = [ "1" ] with Not_found -> false
        in
        let sync =
          try sync_behaviour_of_string @@ List.hd @@ List.assoc "sync" query
          with Not_found -> None
        in
        let lock =
          try List.assoc "lock" query = [ "1" ] with Not_found -> false
        in
    ...
    | _ ->
        Error
          (`Msg
            "Config.to_string expected a string of the form \
             file://<path>?sync=(none|os|drive)&buffered=(0|1)&lock=(0|1)")
end
...
type t = {
  ...
  size_bytes : int64;
  (* used to handle the last sector, if the file isn't a multiple *)
  config : Config.t;
...
let ( >>*= ) m f =
  m >>= function Ok x -> f x | Error x -> Lwt.return (Error x)

let stat _filename fd =
  Rresult.R.trap_exn Unix.LargeFile.fstat fd
...
let get_file_size filename fd =
  ...
  | _ ->
      Log.err (fun f ->
          f "get_file_size %s: entity is neither a file nor a block device"
            filename);
      Lwt.return
      @@ Error
           (`Msg
             (Printf.sprintf
                "get_file_size %s: neither a file nor a block device" filename))

let get_sector_size filename fd =
  stat filename fd >>*= fun st ->
  match st.Unix.LargeFile.st_kind with
  | Unix.S_REG ->
      Lwt.return @@ Ok 512 (* FIXME: no easy way to determine this *)
  | Unix.S_BLK -> Lwt.return @@ blkgetsectorsize fd
  | _ ->
      Log.err (fun f ->
          f "get_sector_size %s: entity is neither a file nor a block device"
            filename);
      Lwt.return
      @@ Error
           (`Msg
             (Printf.sprintf
                "get_sector_size %s: neither a file nor a block device"
                filename))

let of_config ({ Config.buffered; path; lock; sync = _ } as config) =
  let openfile, use_fsync_after_write =
    match (buffered, is_win32) with
    ...
    | false, true ->
        ...
        (Raw.openfile_buffered, true)
  in
  (* first try read/write and then fall back to read/only *)
  try
    let fd, read_write =
      try (openfile path true 0o0, true)
      with _ -> (openfile path false 0o0, false)
    in
    ...
    | Ok size_bytes -> (
        ...
        | Ok sector_size ->
            ...
            let size_sectors =
              Int64.(
                div
                  (add size_bytes (of_int (sector_size - 1)))
                  (of_int sector_size))
            in
            if
              Int64.(mul size_sectors (of_int sector_size)) > size_bytes
              && not buffered
            then
              Log.warn (fun f ->
                  f
                    "Length not sector aligned: O_DIRECT will fail with EINVAL \
                     on some platforms");
...
let lwt_wrap_exn t op offset ?(buffers = []) f =
  ...
  let describe_buffers buffers =
    if buffers = [] then ""
      ...
      "with buffers of length [ "
      ^ String.concat ", "
          (List.map (fun b -> string_of_int @@ Cstruct.len b) buffers)
      ^ " ]"
  in
  ...
  Lwt_list.fold_left_s
    (fun acc b ->
      ...
      | Ok () ->
          ...
          if len mod t.info.sector_size <> 0 then
            fatalf
              "%s: buffer length (%d) is not a multiple of sector_size (%d) \
               for file %s"
              op len t.info.sector_size t.config.Config.path
          else Lwt.return (Ok ()))
    (Ok ()) buffers
  >>*= fun () ->
  Lwt.catch f (function
    | End_of_file ->
        fatalf "%s: End_of_file at file %s offset %Ld %s" op
          t.config.Config.path offset (describe_buffers buffers)
    | Unix.Unix_error (code, fn, arg) ->
        fatalf "%s: %s in %s '%s' at file %s offset %Ld %s" op
          (Unix.error_message code) fn arg t.config.Config.path offset
          (describe_buffers buffers)
    | e ->
        fatalf "%s: %s at file %s offset %Ld %s" op (Printexc.to_string e)
          t.config.Config.path offset (describe_buffers buffers))

let seek_already_locked x fd offset =
  if x.seek_offset <> offset then (
    x.seek_offset <- offset;
    Lwt_unix.LargeFile.lseek fd offset Unix.SEEK_SET )
  else Lwt.return offset
      ...
      | Some fd ->
          ...
          then (
            ...
            Lwt_mutex.with_lock x.m (fun () ->
                ...
                Lwt.catch
                  (fun () ->
                    if is_win32 || List.length buffers = 1 then
                      let rec loop = function
                        ...
                        | b :: bs ->
                            let virtual_zeroes =
                              Int64.(
                                sub
                                  (add offset (of_int (Cstruct.len b)))
                                  x.size_bytes)
                            in
                            ( if virtual_zeroes <= 0L then really_read fd b
                            else (
                            ...
                            >>= fun () ->
                            x.seek_offset <-
                              Int64.(add x.seek_offset (of_int (Cstruct.len b)));
                            loop bs
                      in
                      loop buffers
                    else
                            ...
                            if Cstructs.len remaining = 0 then Lwt.return_unit
                              ...
                              Lwt_unix.run_job
                                (Raw.readv_job
                                   ...
                                   iovec)
                              >>= fun n -> loop (Cstructs.shift remaining n)
                          in
                          loop buffers)
                        (split_list buffers iov_len)
                      >>= fun () ->
                      x.seek_offset <-
                        Int64.add x.seek_offset (Int64.of_int len);
                      Lwt.return_unit)
                  (fun e ->
                    x.seek_offset <- -1L;
                    (* actual file pointer is undefined now *)
                    Lwt.fail e)
                >>= fun () -> Lwt.return (Ok ())))

let write x sector_start buffers =
  let offset = Int64.(mul sector_start (of_int x.info.sector_size)) in
  lwt_wrap_exn x "write" offset ~buffers (fun () ->
      match x with
      ...
      | { fd = Some fd; _ } ->
          ...
          then (
            Log.err (fun f ->
                f
                  "write beyond end of file: sector_start (%Ld) + len (%d) > \
                   size_sectors (%Ld)"
                  sector_start len_sectors x.info.size_sectors);
            fail End_of_file )
          else
            Lwt_mutex.with_lock x.m (fun () ->
                ...
                Lwt.catch
                  ...
                  (fun e ->
                    ...
                    Lwt.fail e)
                >>= fun () ->
                ( if x.use_fsync_after_write then Lwt_unix.fsync fd
                else Lwt.return () )
                >>= fun () -> Lwt.return (Ok ())))

let resize t new_size_sectors =
  let new_size_bytes =
    Int64.(mul new_size_sectors (of_int t.info.sector_size))
  in
  match t.fd with
  | None -> return (Error `Disconnected)
  | Some fd ->
      lwt_wrap_exn t "ftruncate" new_size_bytes (fun () ->
          Lwt_mutex.with_lock t.m (fun () ->
              ftruncate fd new_size_bytes >>= fun () ->
              t.info <- { t.info with size_sectors = new_size_sectors };
              return (Ok ())))

external flush_job : Unix.file_descr -> bool -> unit Lwt_unix.job
  = "mirage_block_unix_flush_job"

let flush t =
  match t.fd with
  | None -> return (Error `Disconnected)
  | Some fd ->
      lwt_wrap_exn t "fsync" 0L (fun () ->
          ( match t.config.Config.sync with
          | None -> Lwt.return_unit
          | Some `ToOS ->
              Lwt_unix.run_job (flush_job (Lwt_unix.unix_file_descr fd) false)
          | Some `ToDrive ->
              Lwt_unix.run_job (flush_job (Lwt_unix.unix_file_descr fd) true) )
          >>= fun () -> return (Ok ()))
...
let seek_mapped t from =
  ...
  | Some fd ->
      ...
      lwt_wrap_exn t "seek_mapped" offset (fun () ->
          Lwt_mutex.with_lock t.m (fun () ->
...
let seek_unmapped t from =
  ...
  | Some fd ->
      let offset = Int64.(mul from (of_int t.info.sector_size)) in
      lwt_wrap_exn t "seek_unmapped" offset (fun () ->
          Lwt_mutex.with_lock t.m (fun () ->
              let fd = Lwt_unix.unix_file_descr fd in
...
external discard_job : Unix.file_descr -> int64 -> int64 -> unit Lwt_unix.job
  = "mirage_block_unix_discard_job"
