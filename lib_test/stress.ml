...
module Make (B : DISCARDABLE) = struct
  ...
  let random_write_discard stop_after is_empty block =
    ...
    let i =
      SectorSet.Interval.make 0L (Int64.pred info.Mirage_block.size_sectors)
    in
    ...
    let undefined, empty =
      if is_empty then (ref none, ref all)
        (* everything is empty because we created the file *)
      else (ref all, ref none)
    in
    let nr_iterations = ref 0 in

    let buffer_size = 1048576 in
    (* perform 1MB of I/O at a time, maximum *)
    let buffer_size_sectors =
      Int64.of_int (buffer_size / info.Mirage_block.sector_size)
    in
    ...
    let write x n =
      ...
      let one_write x n =
        ...
        let buf =
          Cstruct.sub write_buffer 0
            (Int64.to_int n * info.Mirage_block.sector_size)
        in
        let rec for_each_sector x remaining =
          if Cstruct.len remaining = 0 then ()
          else
        ...
        B.write block x [ buf ] >>= function
        | Error _ -> failwith "write"
        | Ok () -> Lwt.return_unit
      in
      let rec loop x n =
        if n = 0L then Lwt.return_unit
        else
          let n' = min buffer_size_sectors n in
          one_write x n' >>= fun () -> loop (Int64.add x n') (Int64.sub n n')
      in
      loop x n >>= fun () ->
      if n > 0L then (
    ...
    let check_contents sector buf expected =
      ...
      if actual <> expected then
        failwith
          (Printf.sprintf
             "contents of sector %Ld incorrect: expected %Ld but actual %Ld"
             sector expected actual)
    in
    let check_all_clusters () =
      let rec check p set =
        match SectorSet.choose set with
        | i ->
            ...
            let one_read x n =
              assert (n <= buffer_size_sectors);
              let buf =
                Cstruct.sub read_buffer 0
                  (Int64.to_int n * info.Mirage_block.sector_size)
              in
              B.read block x [ buf ] >>= function
              ...
              | Ok () ->
                  let rec for_each_sector x remaining =
                    if Cstruct.len remaining = 0 then ()
                      ...
                      for_each_sector (Int64.succ x)
                        (Cstruct.shift remaining 512)
                  in
                  for_each_sector x buf;
                  Lwt.return_unit
            in
            ...
            in
            loop x n >>= fun () -> check p (SectorSet.remove i set)
    ...
    let rec loop () =
      incr nr_iterations;
      if !nr_iterations = stop_after then Lwt.return_unit
      else
        ...
        ( ( if 0 <= r && r < 20 then (
            ...
            Lwt.pick
              [
                discard sector n;
                ( Lwt_unix.sleep 30. >>= fun () ->
                  Lwt.fail (Failure "discard timeout") );
              ] )
        >>= fun () -> check_all_clusters () )
        >>= fun () -> loop ()
    in
    Lwt.catch loop (fun e ->
        ...
        Lwt_io.open_file
          ...
          ~perm:0o644 ~mode:Lwt_io.output "/tmp/written.sexp"
        >>= fun oc ->
        ...
        Lwt_io.open_file
          ~flags:[ Unix.O_CREAT; Unix.O_TRUNC; Unix.O_WRONLY ]
          ~perm:0o644 ~mode:Lwt_io.output "/tmp/empty.sexp"
        >>= fun oc ->
        Lwt_io.write oc s >>= fun () ->
        Lwt_io.close oc >>= fun () ->
        ...
        Lwt_io.open_file
          ~flags:[ Unix.O_CREAT; Unix.O_TRUNC; Unix.O_WRONLY ]
          ~perm:0o644 ~mode:Lwt_io.output "/tmp/undefined.sexp"
        >>= fun oc ->
        Lwt_io.write oc s >>= fun () ->
        Lwt_io.close oc >>= fun () -> Lwt.fail e)
end
...
let create_file path nsectors =
  ...
  Lwt_unix.openfile path
    [ Unix.O_CREAT; Unix.O_TRUNC; Lwt_unix.O_WRONLY ]
    0o0644
  >>= fun fd ->
  Block.ftruncate fd (Int64.mul nsectors 512L) >>= fun () -> Lwt_unix.close fd
...
let _ =
  ...
  Arg.parse
    [
      ( "-path",
        Arg.Set_string path,
        "Path of file or block device (default: create a fresh file)" );
      ( "-sectors",
        Arg.Set_int sectors,
        Printf.sprintf "Total number of sectors (default %d)" !sectors );
      ( "-stop-after",
        Arg.Set_int stop_after,
        Printf.sprintf
          "Number of iterations to stop after (default: 1024, 0 means never)" );
      ("-debug", Arg.Set debug, "enable debug");
    ]
    (fun x ->
      Printf.fprintf stderr "Unexpected argument: %s\n" x;
      exit 1)
    "Perform random read/write/discard/compact operations on a file or block \
     device";
  ...
  Lwt_main.run
    ...
    ( if not path_already_exists then create_file path sectors
    else Lwt.return_unit )
    >>= fun () ->
    Block.connect ~buffered:false path >>= fun block ->
    Lwt.catch
      (fun () ->
        ...
        if not path_already_exists then Lwt_unix.unlink path
        else Lwt.return_unit)
      (fun e ->
