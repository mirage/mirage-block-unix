...
let copy ~src ~dst ~bs ~buffered ~threads =
  let open Lwt.Infix in
  Block.connect ~buffered src
  >>= fun src ->
  (* Create the destination if it doesn't exist *)
  Lwt_unix.openfile dst [ Lwt_unix.O_CREAT; Lwt_unix.O_WRONLY ] 0o0644
  >>= fun fd ->
  Lwt_unix.close fd
  >>= fun () ->
...
let _ =
  ...
  Lwt_main.run @@ copy ~src:(!src) ~dst:(!dst) ~bs:(!bs) ~buffered:(!buffered) ~threads:(!threads)
